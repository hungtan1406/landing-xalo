...
const QuizPage = () => {
  ...
  const fetchQuestions = useCallback(async () => {
    ...
    try {
      ...
      const questionsWithOptions: QuizQuestion[] = questionsData.map(
        (q: any) => {
          // XỬ LÝ OPTIONS (JSONB) VÀ SHUFFLE

          // Thêm ID tạm thời cho mỗi option để dùng cho RadioGroup (value)
          const optionsWithId: OptionWithId[] = q.options.map(
    ...
    return (
      <div className='min-h-screen flex flex-col bg-[#8B9DFF] font-sans text-black'>
        <div className='flex-grow flex flex-col items-center justify-center p-4 text-center'>
          <h1 className='font-primary text-5xl md:text-8xl font-black uppercase tracking-tighter mb-6 leading-none'>
            WELCOME TO OUR QUIZ
          </h1>

          <p className='text-base md:text-lg mb-12 max-w-2xl font-medium leading-relaxed'>
            Hãy trả lời thật lòng dựa trên những gì bạn có thể quan sát hoặc
            được đánh giá.
            <br className='hidden md:block' />
            Đừng để định kiến cá nhân ảnh hưởng — chỉ cần chọn điều phản ánh
            đúng bạn nhất.
          </p>
  ...
  if (showResultReady) {
    return (
      <div className='min-h-screen flex flex-col bg-[#8B9DFF] font-sans text-black'>
        <div className='flex-grow flex flex-col items-center justify-center p-4 text-center'>
          <h1 className='text-6xl md:text-8xl font-primary font-bold uppercase tracking-tighter mb-6 leading-none'>
            KẾT QUẢ CỦA BẠN ĐÃ SẴN SÀNG
          </h1>

          <p className='text-base font-primary md:text-xl mb-12 max-w-2xl font-medium leading-relaxed'>
          ...
          <Button
            onClick={() =>
              navigate('/results', { state: { result: calculatedResult } })
            }
            className='bg-black font-primary hover:bg-gray-800 text-white font-bold py-6 px-12 rounded-[5px] shadow-lg text-lg uppercase'
          >
            XEM KẾT QUẢ
          </Button>
        </div>
    ...
    return (
      <div className='min-h-screen flex items-center justify-center bg-white p-4'>
        <p className='text-lg text-gray-700'>
          Chưa có câu hỏi nào được tạo. Vui lòng kiểm tra dữ liệu Supabase.
        </p>
      </div>
    );
  }
      ...
      <div className='w-full max-w-3xl mx-auto'>
        ...
        <Card className='w-full shadow-xl rounded-2xl border-0 overflow-hidden'>
          <CardContent className='p-8 md:p-12'>
            <p className='text-xl md:text-2xl font-bold text-center mb-8 text-gray-900 leading-relaxed'>
              ...
              {currentQuestion.options.map((option, index) => (
                ...
                >
                  ...
                  <Label
                    ...
                    className='text-lg text-gray-700 cursor-pointer flex-1 font-medium'
                  >
                    {option.text}
                  </Label>
                </div>
          ...
          <CardFooter className='bg-gray-50 p-6 flex justify-between border-t border-gray-100'>
            ...
            <Button
              ...
              disabled={
                userAnswers[currentQuestion.id] === undefined || submitting
              }
